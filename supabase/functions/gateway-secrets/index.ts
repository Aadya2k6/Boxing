    ...
    if (action === "initiate-payu-payment") {
      ...
      return new Response(
        JSON.stringify({
          ...
          txnid,
          amount,
          productinfo: productinfo ?? "Cricket Academy Fee",
          firstname: firstname ?? "",
          email: email ?? "",
          hash,
          surl: `${Deno.env.get("SITE_URL") ?? "https://app.crickos.com"}/payment/success`,
          furl: `${Deno.env.get("SITE_URL") ?? "https://app.crickos.com"}/payment/failure`,
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }
