...
function AdminJudges() {
  ...
  async function loadData() {
    ...
    try {
      ...
      let invQuery = supabase.from("external_judge_invites").select(`
        *,
        academy:academy_id(id, name),
        tournament:tournament_template_id(id, name)
      `).order("invited_at", { ascending: false });

      let acsQuery = supabase.from("academies").select("id, name");

      if (targetAcademyId) {
        invQuery = invQuery.eq("academy_id", targetAcademyId);
